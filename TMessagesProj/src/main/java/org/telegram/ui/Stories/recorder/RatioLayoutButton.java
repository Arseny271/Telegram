...
public class RatioLayoutButton extends ToggleButton2 {
    public RatioLayoutButton(Context context) {
        super(context);
    }
    
    public static class RatioLayoutListView extends FrameLayout {

        public final RecyclerListView listView;

        private Ratio selectedRatio;

        public void setSelected(Ratio ratio) {
            selectedRatio = ratio;
            AndroidUtilities.updateVisibleRows(listView);
        }

        public RatioLayoutListView(Context context, final FlashViews flashViews) {
            super(context);
            ...
            listView.setAdapter(new RecyclerView.Adapter() {
                ...
                public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
                    ...
                    final Ratio ratio = Ratio.valueOf(position);
                    imageView.setDrawable(new RatioLayoutDrawable(ratio));
                    imageView.setSelected(ratio == selectedRatio, animated);
                ...
                public void onViewAttachedToWindow(@NonNull RecyclerView.ViewHolder holder) {
                    ...
                    if (button.position >= 0 && button.position < getItemCount()) {
                        final Ratio ratio = Ratio.valueOf(button.position);
                        button.setDrawable(new RatioLayoutDrawable(Ratio.valueOf(button.position)));
                        button.setSelected(ratio == selectedRatio, false);
                    }
            ...
            listView.setOnItemClickListener((view, position) -> {
                if (onLayoutClick != null) {
                    onLayoutClick.run(Ratio.valueOf(position));
                }
            });
            addView(listView, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, 56, Gravity.CENTER_HORIZONTAL));
        }
        ...
        private Utilities.Callback<Ratio> onLayoutClick;
        public void setOnLayoutClick(Utilities.Callback<Ratio> onLayoutClick) {
    ...
    public static class RatioLayoutDrawable extends Drawable {
        ...
        public final AnimatedTextView.AnimatedTextDrawable textDrawable = new AnimatedTextView.AnimatedTextDrawable(true, false, false) {
            ...
            public void invalidateSelf() {
                RatioLayoutDrawable.this.invalidateSelf();
            }
        };

        public RatioLayoutDrawable(final Ratio ratio) {
            textDrawable.setText(ratio.toText(), false);
            textDrawable.setTypeface(AndroidUtilities.getTypeface("fonts/num.otf"));
            textDrawable.setTextSize(dpf2(10));
            ...
            clipPath.addRect(-dp(32), -dp(32), dp(32), dp(32), Path.Direction.CW);
            clipPath.addRect(-ow - p, -textDrawable.getTextSize() / 2f, ow + p, textDrawable.getTextSize() / 2f, Path.Direction.CCW);
            clipPath.close();

            final CollageLayout layout = new CollageLayout(".");
